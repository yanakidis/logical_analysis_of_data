                                               ...
                                               bool optim)
{
    ...
    for (auto it=concepts.begin(); it != concepts.end(); it++)
    {
        ...
        {
            auto tmp = bitset_diff((*it).second, Y[i]);
...
// дсм-метод, возвращающий y_proba (y_proba_golos) и время fit'a
// если optim == true, то транспонируем матрицу
                                                                  ...
                                                                  bool optim=true)
{
    ...
    for (int i=0; i!=X_test.size(); i++)
    {
        ...
        {
            auto tmp = bitset_diff( (*it), X_test[i]);
    ...
    for (int i=0; i!= X_test.size(); i++)
    {
        ...
        if (count_pos == count_neg)
        {
            ...
            y_proba_golos[i] = count_pos/(count_pos + count_neg); // вероятность положительного класса
        }
    }
...
int main(int argc, const char * argv[])
{
    for (int i=0; i<10; i++)
    {
        ...
        // сюда подаем обучающую выборку
        train.open("datasets_bin/bin_chess/bin_chess" + to_string(i), ios::in);
        ...
        while (1)
        {
            ...
            // здесь вместо "nowin" и "won" пишем лейблы классов
            // причем лейблы должны отличаться от 0 и 1 !!!
            if (not (s == "0" || s == "1" || s == "nowin" || s== "won"))
                continue;
        ...
        // сюда подаем тестовую выборку
        test.open("datasets_bin/bin_chess/bin_chess_test" + to_string(i), ios::in);
        ...
        tie(y_proba, y_proba_golos, time, pos_size, neg_size) = jsm(X_train, X_test, y_train, true);

        fstream res;

        // директория для результата
        res.open("datasets_bin/bin_chess_res/res" + to_string(i), ios::out);
